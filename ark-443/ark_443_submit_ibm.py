"""
ARK-443 — Principal 8-arm job submission
Remnant Fieldworks Inc. — Derek Hone

Two-of-Three Quorum Authorization on ibm_marrakesh (Heron r2).

ORDERING CONSTRAINT (enforced in code, Field 27):
  1. spam_results.json MUST exist and its SPAM gate MUST have passed
     (SPAM_baseline <= 0.02 on ALL of Q_P, Q_A1, Q_A2, Q_A3). Otherwise this
     script ABORTS without submitting anything.
...
from ark_443_circuits import build_all_arms, INITIAL_LAYOUT, ARM_NAMES, ARM_CLASS, ARM_EXPECT
...
def check_spam_gate():
    ...
    if not os.path.exists(spam_path):
        print("[SUBMIT] ABORT: spam_results.json not found. Run ark_443_spam_job.py first.")
        sys.exit(2)
...
def record_run_log(job_id, submitted, spam, tqc_depths):
    ...
    lines.append(f"- **SPAM baselines:** "
                 f"Q_P={m['Q_P']['spam_baseline']:.4f}, "
                 f"Q_A1={m['Q_A1']['spam_baseline']:.4f}, "
                 f"Q_A2={m['Q_A2']['spam_baseline']:.4f}, "
                 f"Q_A3={m['Q_A3']['spam_baseline']:.4f}\n")
...
def main():
    ...
    print("[SUBMIT] job submitted and recorded. Use ark_443_retrieve.py to fetch results.")
