"""
ARK-445b — Principal job result retrieval
Tri-State Authorization Discrimination (ALLOW / HOLD / DENY)
Remnant Fieldworks Inc. — Derek Hone

Retrieves raw counts for the principal 9-arm job and writes raw_results.json.
Job id is read from principal_job_meta.json (recorded at submission time), so results
are only ever read AFTER the job id has been committed.

Raw counts are the PRIMARY endpoint. No readout mitigation is applied.
The payload readout is the classical register 'cp'; the authorization register 'ca'
is captured for diagnostic cross-checks (shot-by-shot Q_A vs Q_P correlation in HOLD).
"""
...
def main():
    ...
    for i, name in enumerate(arm_order):
        counts = result[i].data.cp.get_counts()    # payload register 'cp' (PRIMARY)
        total = sum(counts.values())
        p1 = counts.get("1", 0) / total
        rec = {"counts": counts, "total": total, "P_Q_P_1": p1}
        # Diagnostic: authorization register 'ca' (recorded authorization bit).
        try:
            ca_counts = result[i].data.ca.get_counts()
            rec["ca_counts"] = ca_counts
        except Exception:
            rec["ca_counts"] = None
        arms[name] = rec
    ...
    out = {
        ...
        "arm_order": arm_order,
        "arm_class": meta.get("arm_class"),
        "arm_expect": meta.get("arm_expect"),
