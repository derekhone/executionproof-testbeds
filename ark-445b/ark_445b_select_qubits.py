"""
ARK-445b — Qubit Selection (execution-time) on ibm_marrakesh
Tri-State Authorization Discrimination (Reset-Free Retest)
Remnant Fieldworks Inc. — Derek Hone

Selects 2 physical qubits (Q_A authorizer, Q_P payload) for the 8-arm reset-free tri-state test.

Selection rule (identical to ARK-445, Field 5.2):
  1. Readout error (RE) < 0.02 for BOTH qubits.
  2. Q_A and Q_P must be CONNECTED (a native 2-qubit gate exists between them,
     though ARK-445b uses no inter-qubit gate — classical feedforward only).
...
Outputs (committed BEFORE any job):
  ...
  - calibration_snapshot_ibm_marrakesh_YYYYMMDD.json : backend calibration snapshot
"""
...
def select_qubits(backend):
    ...
    if not candidates:
        raise ValueError(
            ...
            "ARK-445b requires 2 connected low-readout-error qubits.")
