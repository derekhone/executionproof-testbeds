"""
ARK-445b — In-situ SPAM kill-gate job (RUNS FIRST) on ibm_marrakesh
Tri-State Authorization Discrimination (ALLOW / HOLD / DENY)
Remnant Fieldworks Inc. — Derek Hone

Directly characterizes idle SPAM readout error on the ARK-445b rule-selected physical
qubits Q_A and Q_P of ibm_marrakesh, BEFORE the principal 8-arm job (Field 13 step 2).
...
from ark_445b_circuits import arm9_spam_idle, INITIAL_LAYOUT
...
def main():
    ...
    print(f"[SPAM] submitting arm9_spam_idle x {SHOTS} shots to {BACKEND_NAME} "
          f"(qubits={physical}) ...")
