"""
ARK-446 — Analysis and scoring
Remnant Fieldworks Inc. — Derek Hone

Cross-device replication of ARK-441 on ibm_marrakesh (Heron r2).
...
Also computes H2d cross-device concordance vs the ARK-441 result on
ibm_kingston (marrakesh point estimates checked against ARK-441 95% Wilson CIs).
...
(both near the readout floor); that is a PASS, not a kill.
"""
...
ARK441_PROOF = os.path.join(HERE, "..", "ark-441", "proofrecord.json")
...
def main():
    ...
    # H2d — cross-device concordance vs ARK-441 (ibm_kingston).
    # marrakesh point estimates checked against ARK-441 95% Wilson CIs.
    concordance = None
    if os.path.exists(ARK441_PROOF):
        with open(ARK441_PROOF) as f:
            ark441 = json.load(f)
        a441m = ark441["metrics"]
        checks = {}
        # L_D, S_A, L_control point estimates vs ARK-441 95% CI
        for key, this_val in [("L_D", L_D), ("S_A", S_A),
                              ("L_control", L_control)]:
            lo, hi = a441m[key]["ci95"][1], a441m[key]["ci95"][2]
            checks[key] = {
                "marrakesh_point": this_val,
                "ark441_ci95": [lo, hi],
                "within_ark441_ci95": bool(lo <= this_val <= hi),
            }
        # SPAM baseline (arm4) concordance
        lo, hi = a441m["SPAM_baseline_insitu_arm4"]["ci95"][1], a441m["SPAM_baseline_insitu_arm4"]["ci95"][2]
        checks["SPAM_baseline_insitu_arm4"] = {
            "marrakesh_point": SPAM_baseline,
            "ark441_ci95": [lo, hi],
            "within_ark441_ci95": bool(lo <= SPAM_baseline <= hi),
        }
        concordance = {
            "reference_experiment": "ARK-441",
            "reference_backend": ark441.get("backend"),
            "reference_verdict": ark441.get("decision_outcome"),
            "checks": checks,
            "note": ("H2d: cross-device concordance is REPORTED, not a decision gate. "
                     "Point estimates near the readout floor may fall just outside a "
                     "tight CI without contradicting the qualitative replication "
                     "(same PASS verdict, same boundary behaviour)."),
        }

    proofrecord = {
        "experiment_id": "ARK-446",
        "candidate": "A — SPAM-Resolved Authorization Boundary Characterization (cross-device replication)",
        "backend": raw["backend"],
        ...
        "cross_device_concordance_H2d": concordance,
...
def _make_plots(raw, ci95, S_A, L_D, SPAM_baseline, L_D_corrected):
    ...
    ax.set_title("ARK-446 (ibm_marrakesh) — Payload activation by arm (95% Wilson CI)")
    ...
    ax.set_title("ARK-446 (ibm_marrakesh) — SPAM decomposition of DENY leakage")
    ax.legend()
    fig.tight_layout()
