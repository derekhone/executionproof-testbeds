...
from generator.scenario_generator import ScenarioGenerator, dependency_loss_effective, SEED_BASE, ARMS

BASELINE_ARM = 1
DEGRADED_ARMS = [2, 3, 4, 5, 6, 7, 8]

def run_arm(arm: int) -> dict:
    """Run a single arm: generate, check effectiveness, evaluate with both guards."""
    print("=" * 70)
    print(f"ARM {arm}: {ARMS[arm]['label']} (expect {ARMS[arm]['expected']})")
    print("=" * 70)
    
    # [1/4] Generate scenarios
    ...
    print("[2/4] Checking dependency-loss effectiveness...")
    failures = []
    for sc in scenarios:
    ...
    if failures:
        print(f"\n✗ ARM {arm} ABORT: dependency-loss effectiveness gate failed")
        sys.exit(1)
    print(f"  ✓ All scenarios dependency-loss-effective")
    
    # [3/4] Evaluate with V1
...
def compute_overall(arm_results: list) -> dict:
    ...
    # Metrics
    g_allow_baseline = by_arm[BASELINE_ARM]["rates"]["allow"]
    g_deny_degraded_min = min(by_arm[a]["rates"]["deny"] for a in DEGRADED_ARMS)
    g_margin = min(g_allow_baseline, g_deny_degraded_min) - 0.05
    
    # Criteria
    c1_pass = g_allow_baseline >= 0.95
    c2_pass = g_deny_degraded_min >= 0.95
    ...
    print(f"\nPrimary Metrics:")
    print(f"  g_allow_baseline (arm {BASELINE_ARM} ALLOW):      {g_allow_baseline:.4f}")
    print(f"  g_deny_degraded_min (arms 2-8 DENY):  {g_deny_degraded_min:.4f}")
    ...
    print(f"\nPass Criteria:")
    print(f"  C1 (g_allow_baseline ≥ 0.95):     {_mark(c1_pass)}")
    print(f"  C2 (g_deny_degraded_min ≥ 0.95):  {_mark(c2_pass)}")
    ...
    overall_result = {
        ...
        "primary_metrics": {
            "g_allow_baseline": round(g_allow_baseline, 4),
            "g_deny_degraded_min": round(g_deny_degraded_min, 4),
            "g_margin": round(g_margin, 4),
        },
        "criteria": {
            "c1_allow_baseline_pass": c1_pass,
            "c2_deny_degraded_pass": c2_pass,
