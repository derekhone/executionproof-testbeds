#!/usr/bin/env python3
"""
ARK-457 Kill-Gate Calibration
Verify dual-guard concordance and context-replay effectiveness before running arms.
"""
import json
import subprocess
import sys
from pathlib import Path
from generator.scenario_generator import ScenarioGenerator, context_replay_effective, SEED_BASE


def main():
    print("=" * 70)
    print("ARK-457 KILL-GATE CALIBRATION")
    print("=" * 70)

    # [1/4] Generate calibration scenarios (mixed across all arms)
    print("\n[1/4] Generating calibration scenarios (all 8 arms)...")
    scenarios = []
    ...
    # [2/4] Check context-replay effectiveness
    print("\n[2/4] Checking context-replay effectiveness...")
    failures = []
    for sc in scenarios:
        is_eff, reason = context_replay_effective(sc, sc["arm"])
        if not is_eff:
    ...
    if failures:
        ...
        for sid, reason in failures:
            print(f"    {sid}: {reason}")
        print("\n✗ KILL-GATE ABORT: context-replay effectiveness failed")
        sys.exit(1)
    print(f"  ✓ All {len(scenarios)} scenarios are context-replay-effective")
