#!/usr/bin/env python3
"""
ARK-459 — Cloud IAM Role Grant · Revocation At Execution (scenario generator).

Combines ARK-458's 5-dim IAM action tuple with ARK-451's timeline/revocation model.
...
Timeline model:
  - t_approval: when the IAM grant is approved (≥ 0)
...
# IAM action dimensions (from ARK-458)
IAM_PRINCIPALS = [
...
def _iam_action(rng: random.Random) -> Dict[str, str]:
    """Generate a random IAM action tuple."""
    return {
