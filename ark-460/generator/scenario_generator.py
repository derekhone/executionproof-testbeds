#!/usr/bin/env python3
"""
ARK-460 — Cloud IAM Role Grant · Dependency Loss (scenario generator).
...
Model: Each scenario has an IAM action tuple (exact-match, from ARK-458) plus a
...
CRITICAL_DEPS = ["PolicyRegistry", "IdentityProvider", "SignatureVerifier", "AuditLogger"]

def _hash_scenario_id(arm: int, idx: int) -> str:
    s = f"ARK-460-arm{arm}-{idx}"
...
def _dependencies(rng: random.Random, arm: int) -> Dict[str, str]:
    ...
    Returns: {"PolicyRegistry": "AVAILABLE"|"UNAVAILABLE", ...}
    """
    if arm == 1:
        ...
        # ARM 2: PolicyRegistry UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["PolicyRegistry"] = "UNAVAILABLE"
        return deps
    
    elif arm == 3:
        # ARM 3: IdentityProvider UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["IdentityProvider"] = "UNAVAILABLE"
        return deps
    
    elif arm == 4:
        # ARM 4: SignatureVerifier UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["SignatureVerifier"] = "UNAVAILABLE"
        return deps
    
    elif arm == 5:
        # ARM 5: AuditLogger UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["AuditLogger"] = "UNAVAILABLE"
        return deps
...
def _dependency_gate_oracle(dependencies: Dict[str, str], arm: int) -> bool:
    ...
    if arm in [1, 8]:
        ...
        # PolicyRegistry UNAVAILABLE, others available
        return (dependencies["PolicyRegistry"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "PolicyRegistry"))
    elif arm == 3:
        # IdentityProvider UNAVAILABLE, others available
        return (dependencies["IdentityProvider"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "IdentityProvider"))
    elif arm == 4:
        # SignatureVerifier UNAVAILABLE, others available
        return (dependencies["SignatureVerifier"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "SignatureVerifier"))
    elif arm == 5:
        # AuditLogger UNAVAILABLE, others available
        return (dependencies["AuditLogger"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "AuditLogger"))
    elif arm == 6:
