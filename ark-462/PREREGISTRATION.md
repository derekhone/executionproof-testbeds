...
- **Commercial purpose:** Answers **Prospect Question #7** — *"Can a high-risk action requiring human approval proceed without it?"* — in the AWS-style IAM context. High-privilege IAM grants (AdminFull, SecurityOfficer) often require human approval. A correct gate must **fail safe** (HOLD when approval required but not yet received), never wrongly ALLOW.
