#!/usr/bin/env python3
"""
ARK-464 — Production Deployment · Revocation At Execution (scenario generator).

Combines ARK-458's 5-dim deployment action tuple with ARK-451's timeline/revocation model.
...
Timeline model:
  - t_approval: when the deployment is approved (≥ 0)
...
# deployment action dimensions (from ARK-458)
IAM_PRINCIPALS = [
    f"arn:aws:iam::{10**11 + i:012d}:user/svc-{i:04d}"
    for i in range(100)
]
IAM_ROLES = [
    "ReadOnlyAuditor", "DataAnalyst", "DeploymentManager", "AdminFull",
...
def _iam_action(rng: random.Random) -> Dict[str, str]:
    """Generate a random deployment action tuple."""
    return {
