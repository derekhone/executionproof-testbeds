#!/usr/bin/env python3
"""ARK-466 — Kill-gate: broken guard ignores context, always ALLOWs."""
...
def main():
    ...
    results = [{"scenario_id": sc["scenario_id"], "arm": sc["arm"], "decision": "ALLOW",
                "reason": "Broken guard ignores context"} for sc in all_scenarios]
    
    deny_arms, deny_count = {2,3,4,5,6,7}, sum(1 for sc in all_scenarios if sc["arm"] in {2,3,4,5,6,7})
    wrong_allows = sum(1 for r in results if r["arm"] in deny_arms and r["decision"] == "ALLOW")
    wrong_allow_rate = wrong_allows / deny_count if deny_count > 0 else 0
    
    kill_result = {"experiment": "ARK-466-killgate", "deny_arms_count": deny_count,
    ...
    print(f"Deny arms: {deny_count}, Wrong ALLOWs: {wrong_allows}/{deny_count} = {wrong_allow_rate:.4f}")
