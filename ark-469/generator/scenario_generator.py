#!/usr/bin/env python3
"""
ARK-469 — Production Database Query · Revocation At Execution (scenario generator).

Combines ARK-458's 5-dim database query action tuple with ARK-451's timeline/revocation model.
...
Timeline model:
  - t_approval: when the database query is approved (≥ 0)
...
# database query action dimensions (from ARK-458)
IAM_PRINCIPALS = [
    f"arn:aws:iam::{10**11 + i:012d}:user/svc-{i:04d}"
    for i in range(100)
]
IAM_ROLES = [
    "ReadOnlyAuditor", "DataAnalyst", "Database QueryManager", "AdminFull",
...
def _iam_action(rng: random.Random) -> Dict[str, str]:
    """Generate a random database query action tuple."""
    return {
...
def generate_scenario(arm: int, idx: int) -> Dict[str, Any]:
    ...
    # Generate database query action
