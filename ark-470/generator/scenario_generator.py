#!/usr/bin/env python3
"""
ARK-470 — Production Database Query · Dependency Loss (scenario generator).
...
Model: Each scenario has an database query action tuple (exact-match, from ARK-458) plus a
...
IAM_ROLES = ["ReadOnlyAuditor", "DataAnalyst", "Database QueryManager", "AdminFull",
...
CRITICAL_DEPS = ["QueryValidator", "SchemaRegistry", "PermissionStore", "Database QueryAuditor"]

def _hash_scenario_id(arm: int, idx: int) -> str:
    s = f"ARK-470-arm{arm}-{idx}"
...
def _dependencies(rng: random.Random, arm: int) -> Dict[str, str]:
    ...
    Returns: {"QueryValidator": "AVAILABLE"|"UNAVAILABLE", ...}
    """
    if arm == 1:
        ...
        # ARM 2: QueryValidator UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["QueryValidator"] = "UNAVAILABLE"
        return deps
    
    elif arm == 3:
        # ARM 3: SchemaRegistry UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["SchemaRegistry"] = "UNAVAILABLE"
        return deps
    
    elif arm == 4:
        # ARM 4: PermissionStore UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["PermissionStore"] = "UNAVAILABLE"
        return deps
    
    elif arm == 5:
        # ARM 5: Database QueryAuditor UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["Database QueryAuditor"] = "UNAVAILABLE"
        return deps
...
def _dependency_gate_oracle(dependencies: Dict[str, str], arm: int) -> bool:
    ...
    if arm in [1, 8]:
        ...
        # QueryValidator UNAVAILABLE, others available
        return (dependencies["QueryValidator"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "QueryValidator"))
    elif arm == 3:
        # SchemaRegistry UNAVAILABLE, others available
        return (dependencies["SchemaRegistry"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "SchemaRegistry"))
    elif arm == 4:
        # PermissionStore UNAVAILABLE, others available
        return (dependencies["PermissionStore"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "PermissionStore"))
    elif arm == 5:
        # Database QueryAuditor UNAVAILABLE, others available
        return (dependencies["Database QueryAuditor"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "Database QueryAuditor"))
    elif arm == 6:
