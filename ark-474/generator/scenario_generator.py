#!/usr/bin/env python3
"""
ARK-474 — Production Financial Transaction · Revocation At Execution (scenario generator).

Combines ARK-458's 5-dim financial transaction action tuple with ARK-451's timeline/revocation model.
...
Timeline model:
  - t_approval: when the financial transaction is approved (≥ 0)
...
# financial transaction action dimensions (from ARK-458)
IAM_PRINCIPALS = [
    f"arn:aws:iam::{10**11 + i:012d}:user/svc-{i:04d}"
    for i in range(100)
]
IAM_ROLES = [
    "ReadOnlyAuditor", "DataAnalyst", "Financial TransactionManager", "AdminFull",
...
def _iam_action(rng: random.Random) -> Dict[str, str]:
    """Generate a random financial transaction action tuple."""
    return {
...
def generate_scenario(arm: int, idx: int) -> Dict[str, Any]:
    ...
    # Generate financial transaction action
