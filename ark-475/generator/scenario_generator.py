#!/usr/bin/env python3
"""
ARK-475 — Production Financial Transaction · Dependency Loss (scenario generator).
...
Model: Each scenario has an financial transaction action tuple (exact-match, from ARK-458) plus a
...
IAM_ROLES = ["ReadOnlyAuditor", "DataAnalyst", "Financial TransactionManager", "AdminFull",
...
CRITICAL_DEPS = ["PaymentGateway", "FraudDetector", "AccountVerifier", "Financial TransactionAuditor"]

def _hash_scenario_id(arm: int, idx: int) -> str:
    s = f"ARK-475-arm{arm}-{idx}"
...
def _dependencies(rng: random.Random, arm: int) -> Dict[str, str]:
    ...
    Returns: {"PaymentGateway": "AVAILABLE"|"UNAVAILABLE", ...}
    """
    if arm == 1:
        ...
        # ARM 2: PaymentGateway UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["PaymentGateway"] = "UNAVAILABLE"
        return deps
    
    elif arm == 3:
        # ARM 3: FraudDetector UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["FraudDetector"] = "UNAVAILABLE"
        return deps
    
    elif arm == 4:
        # ARM 4: AccountVerifier UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["AccountVerifier"] = "UNAVAILABLE"
        return deps
    
    elif arm == 5:
        # ARM 5: Financial TransactionAuditor UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["Financial TransactionAuditor"] = "UNAVAILABLE"
        return deps
...
def _dependency_gate_oracle(dependencies: Dict[str, str], arm: int) -> bool:
    ...
    if arm in [1, 8]:
        ...
        # PaymentGateway UNAVAILABLE, others available
        return (dependencies["PaymentGateway"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "PaymentGateway"))
    elif arm == 3:
        # FraudDetector UNAVAILABLE, others available
        return (dependencies["FraudDetector"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "FraudDetector"))
    elif arm == 4:
        # AccountVerifier UNAVAILABLE, others available
        return (dependencies["AccountVerifier"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "AccountVerifier"))
    elif arm == 5:
        # Financial TransactionAuditor UNAVAILABLE, others available
        return (dependencies["Financial TransactionAuditor"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "Financial TransactionAuditor"))
    elif arm == 6:
