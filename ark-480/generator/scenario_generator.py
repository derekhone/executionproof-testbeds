#!/usr/bin/env python3
"""
ARK-480 — Production API Rate Limit · Dependency Loss (scenario generator).
...
Model: Each scenario has an API rate limit action tuple (exact-match, from ARK-458) plus a
...
IAM_ROLES = ["ReadOnlyAuditor", "DataAnalyst", "API Rate LimitManager", "AdminFull",
...
def _dependencies(rng: random.Random, arm: int) -> Dict[str, str]:
    ...
    Returns: {"RateLimiter": "AVAILABLE"|"UNAVAILABLE", ...}
    """
    if arm == 1:
        ...
        # ARM 2: RateLimiter UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["RateLimiter"] = "UNAVAILABLE"
        return deps
    
    elif arm == 3:
        # ARM 3: QuotaStore UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["QuotaStore"] = "UNAVAILABLE"
        return deps
    
    elif arm == 4:
        # ARM 4: TokenValidator UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["TokenValidator"] = "UNAVAILABLE"
        return deps
    
    elif arm == 5:
        # ARM 5: API Rate LimitAuditor UNAVAILABLE → expect HOLD
        deps = {dep: "AVAILABLE" for dep in CRITICAL_DEPS}
        deps["API Rate LimitAuditor"] = "UNAVAILABLE"
        return deps
...
def _dependency_gate_oracle(dependencies: Dict[str, str], arm: int) -> bool:
    ...
    if arm in [1, 8]:
        ...
        # RateLimiter UNAVAILABLE, others available
        return (dependencies["RateLimiter"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "RateLimiter"))
    elif arm == 3:
        # QuotaStore UNAVAILABLE, others available
        return (dependencies["QuotaStore"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "QuotaStore"))
    elif arm == 4:
        # TokenValidator UNAVAILABLE, others available
        return (dependencies["TokenValidator"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "TokenValidator"))
    elif arm == 5:
        # API Rate LimitAuditor UNAVAILABLE, others available
        return (dependencies["API Rate LimitAuditor"] == "UNAVAILABLE" and
                all(dependencies[d] == "AVAILABLE" for d in CRITICAL_DEPS if d != "API Rate LimitAuditor"))
    elif arm == 6:
