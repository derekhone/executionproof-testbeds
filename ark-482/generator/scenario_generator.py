#!/usr/bin/env python3
"""
ARK-482 — Production API Rate Limit · Human Escalation (scenario generator).

Question: When an API rate limit requires human approval, does the gate HOLD
(fail-safe) until the human approval arrives, then ALLOW? Or does it wrongly
ALLOW without waiting?

Model: Each scenario has an API rate limit action + `requires_human_approval` flag +
...
def _hash_scenario_id(arm: int, idx: int) -> str:
    return hashlib.sha256(f"ARK-482-arm{arm}-{idx}".encode()).hexdigest()[:16]
