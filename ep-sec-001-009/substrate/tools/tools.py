...
def perform_side_effect(tool_id: str, parameters: dict) -> str:
    ...
    GUARDED (v1.1-remediated): raises ToolBypassError unless the immediate
    caller is a method of the EnforcementPoint class.  This closes the
    vulnerability discovered in EP-SEC-009 C005 where the function was
    publicly callable without any enforcement check.
    """
    # ---- call-origin guard ------------------------------------------------
    caller_frame = inspect.currentframe().f_back
    try:
        caller_self = caller_frame.f_locals.get("self")
        caller_cls_name = type(caller_self).__name__ if caller_self else None
        caller_module = caller_frame.f_globals.get("__name__", "")
        is_enforcement_point = (
            caller_cls_name == "EnforcementPoint"
            and "enforcement" in caller_module
        )
    finally:
        del caller_frame  # prevent reference-cycle leaks

    if not is_enforcement_point:
        # Record the blocked bypass attempt in the tool ledger.
        entry = build_ledger_entry(
            tool_id, "UNAUTH_BYPASS", idempotency_key=None,
            invocation_type="BYPASS_ATTEMPT",
            gate_decision="DENY-by-design",
            parameters=parameters, proofrecord_id=None)
        append_ledger(entry)
        raise ToolBypassError(
            f"perform_side_effect({tool_id!r}) blocked: caller "
            f"({caller_cls_name} in {caller_module}) is not the "
            f"EnforcementPoint.  All tool execution must pass through "
            f"the governed enforcement path.")
    # ---- guard passed — execute the side effect --------------------------
